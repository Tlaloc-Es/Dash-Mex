...
from dash_mex.charts import generate_parcats


@lru_cache(maxsize=1)
def _get_home_figures():
    """Build expensive home charts once and reuse across requests."""
    ...
    fig_a = generate_parcats(
        nmfc, dimensiones_a, 'Disposicion de los delitos para nueva metodología del fuero común'
    )
    fig_b = generate_parcats(
        nmdf,
        dimensiones_b,
        'Disposicion de los delitos para nueva metodología de incidencia delictiva federal',
    )
    fig_c = generate_parcats(
        vmdf,
        dimensiones_c,
        'Disposicion de los delitos para vieja metodología de incidencia delictiva estatal',
    )
...
def get_index_page():
    fig_a, fig_b, fig_c = _get_home_figures()

    return html.Div(
        [
            dcc.Markdown("""
        # Criminalidad en México
        Esta sección presenta los datos en dos marcos metodológicos (nuevo y previo), cada uno con categorías y subcategorías específicas de delito.
        ...
        * http://www.diputados.gob.mx/documentos/Congreso_Nacional_Legislativo/Doc/CMD_Integrado.pdf
        ...
        Los datos originalmente reportados a nivel municipal se agregan por entidad federativa para mejorar el rendimiento y facilitar la comparación.
        ...
        """),
            dcc.Loading(dcc.Graph(figure=fig_a), type='circle'),
            dcc.Loading(dcc.Graph(figure=fig_b), type='circle'),
            dcc.Loading(dcc.Graph(figure=fig_c), type='circle'),
            dcc.Markdown("""
        # Geo
        ...
        * https://www.inegi.org.mx/app/mapas/
        ...
        Se integran varios datasets para visualizar indicadores clave por entidad federativa y su evolución temporal.
