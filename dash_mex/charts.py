...
def generate_parcats_2(dataset, group, filter=None, filter_value=None, year=None, title=None):
    ...
    fig = go.Figure(
        data=[go.Parcats(dimensions=dimensions, counts=df['Total'])], layout=make_title(title)
    )
...
def cor_indicator(df, filter, filter_value, year, title):
    ...
    if year in dfl['Año'].unique():
        result = (
            result[(result['Año'] == year) & (result[filter] == filter_value)]
            .groupby(['Entidad', filter, 'ile'], as_index=False)['Total']
            .sum()
        )

        corr = result[['Total', 'ile']].corr()
        result = corr.loc['ile', 'Total']
...
def generate_map(
    dataset, filter, filter_value, locations, year, geojson, featureidkey, color, title
):
...
def generate_chart_top(
    ...
    title,
    other_filter=None,
    other_filter_value=None,
    chart=True,
    order=True,
):
    df = dataset.copy()

    if other_filter is not None and other_filter_value is not None:
        df = df[df[other_filter] == other_filter_value]

    df = (
        df[(df['Año'] == year) & (df[filter] == filter_value)]
        .groupby(['Entidad', filter])
        .sum()
        .reset_index()
        .drop([filter, 'Año'], axis=1)
        .sort_values(['Total'], ascending=order)
        .tail(5)
    )

    bar = go.Bar(x=df['Total'], y=df['Entidad'], orientation='h')
    fig = go.Figure(bar, layout=make_title(title)) if chart else bar

    return fig


def generate_comparative_chart_top(
    dataset, filter, filter_value, year, title, other_filter, other_filter_values, order=True
):
    df = dataset.copy()

    fig = make_subplots(rows=1, cols=2, subplot_titles=other_filter_values)

    i = 1
    for value in other_filter_values:
        fig.add_trace(
            generate_chart_top(
                df, filter, filter_value, year, '', other_filter, value, chart=False, order=order
            ),
            row=1,
            col=i,
        )
...
def generate_scatter(dataset, year, filter, filter_value, x, y, x_label, y_label, title):
    ...
    for i, sexo in enumerate(df[y].unique()):
        fig.add_trace(
            go.Scatter(
...
def generate_pies(data, label, value):
    ...
    go.Layout(autosize=True, title=label)
...
def generate_spline_ile(dataset, filter, filter_value, title):
    ...
    fig.add_trace(go.Scatter(x=df['Año'], y=df['Total'], name='Total delito', line_shape='spline'))

    fig.add_trace(
        go.Scatter(
            x=dfl['Año'], y=dfl['ile'], name='Indice libertad economica', line_shape='spline'
        )
    )
...
def generate_spline(dataset, filter, filter_value, value, title):
    ...
    chart = go.Scatter(x=df['Año'], y=df[value], name='spline', line_shape='spline')
...
def generate_comparative_bars(dataset, filter, filter_value, year, title):
    ...
    fig = go.Figure(
        data=[go.Bar(x=df[filter], y=df['Total'], marker_color=colors)], layout=make_title(title)
    )
...
def generate_maps(
    dataset, filter, filter_value, value, locations, geojson, featureidkey, title, split=True
):
    ...
    for i in range(len(years)):
        ...
        layout[geo_key] = dict(visible=False, fitbounds='locations', domain=dict(x=[], y=[]))
