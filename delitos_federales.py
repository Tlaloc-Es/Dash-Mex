...
title = 'Incidencia Delictiva Federal, 2012 - 2019'

fig_a = 'delito-vieja-federal-a'
fig_b = 'delito-vieja-federal-b'
fig_c = 'delito-vieja-federal-c'
fig_d = 'delito-vieja-federal-d'
fig_e = 'delito-vieja-federal-e'

DATA = pd.read_csv('./data/delitos/u_nueva_metodologia_delictiva_federal.csv')
YEARS = DATA['Año'].unique()
MAIN_FILTER = 'Concepto'
...
def update_delito(delito, year, all):
    ...
    if len(all) == 1:
        fig_a = generate_maps(DATA, MAIN_FILTER, delito, 'Total', 'Entidad', STATES, 'properties.NOMGEO', f'Evolución de los casos de {delito} desde {YEARS[0]} hasta {YEARS[-1]}', split=False)
